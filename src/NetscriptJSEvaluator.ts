  ...
  try {
    ...
    walk.simple(ast, {
      ImportDeclaration(node: any) {
        ...
        importNodes.push({ filename: node.source.value, start: node.source.range[0] + 1, end: node.source.range[1] - 1 })
      }
    });
    ...
    for (const node of importNodes) {
      ...
      const importedScriptAbsolutePath = evaluateFilePath(filename, scriptDirectory) ||
        evaluateFilePath(filename + ".js", scriptDirectory) ||
        filename;
      ...
      urlStack.push(...urls);
      const blobUrl = urls[urls.length - 1].url;
      ImportCache.store(importedScript.hash(), urls);

      // Replace the blob inside the import statement.
      transformedCode = transformedCode.substring(0, node.start) + blobUrl + transformedCode.substring(node.end);
    }
